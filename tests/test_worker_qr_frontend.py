...
def test_worker_qr_frontend_uses_png_preview_and_download():
    ...
    assert 'return "/api/worker-qr.png?"' in javascript
    assert "function generateWorkerQrPng()" in javascript
    assert 'workerQrOpenBtn.addEventListener("click", generateWorkerQrPng)' in javascript
    assert "showWorkerQrPreview(workerId)" in javascript
    assert "window.open(\"/api/worker-qr?" not in javascript
