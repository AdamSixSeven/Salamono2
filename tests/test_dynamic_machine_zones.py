...
def test_metric_warning_zone_uses_ground_plane_distance():
    ...
    assert 2.4 < events[0].distance_m < 2.6


def test_metric_danger_zone_has_second_alarm_level():
...
def test_dynamic_machine_zones_are_projected_for_each_machine():
    ...
    assert len(zones) == 2
    assert {zone.severity for zone in zones} == {"WARNING", "DANGER"}
